...
verify-onnx:
    $env:AURA_RUN_SAFETY_INTENT_ONNX="1"; cargo test --locked -p aura-ml --features onnx --test onnx_integration

swift-proto-generate:
    bash scripts/generate-swift-proto.sh --write

swift-proto-check:
    bash scripts/generate-swift-proto.sh --check
...
kids-memory-health-strict:
    cargo run --locked --quiet --example world_sim -p aura-core -- --input crates/aura-core/data/world_sim_kids_memory_stress.json --summary-only --shadow-output artifacts/kids-memory-shadow-run-a.json --require-clean
    cargo run --locked --quiet --example world_sim -p aura-core -- --input crates/aura-core/data/world_sim_kids_memory_stress.json --summary-only --shadow-output artifacts/kids-memory-shadow-run-b.json --require-clean
    cargo run --locked --quiet --example world_sim -p aura-core -- --input crates/aura-core/data/world_lifecycle_suite/kateryna_16_recruitment_pressure_9mo.json --summary-only --shadow-output artifacts/pilot-shadow-run-a.json --require-clean
    cargo run --locked --quiet --example world_sim -p aura-core -- --input crates/aura-core/data/world_lifecycle_suite/kateryna_16_recruitment_pressure_9mo.json --summary-only --shadow-output artifacts/pilot-shadow-run-b.json --require-clean
    cargo run --locked --quiet --example pilot_regression -p aura-core -- --output artifacts/pilot-regression-report.json --require-pass
...
pilot-gate-strict:
    cargo run --locked --example pilot_gate -p aura-core -- --release-report artifacts/release-report.json --pilot-regression-report artifacts/pilot-regression-report.json --shadow-bundle artifacts/pilot-shadow-run-a.json --shadow-bundle artifacts/pilot-shadow-run-b.json --review-signoffs artifacts/pilot-review-signoffs.json --release-revision "$(git rev-parse HEAD)" --kids-memory-health-report artifacts/kids-memory-health.json --kids-preprod-dry-run-report artifacts/kids-preprod-dry-run-matrix.json --output artifacts/pilot-gate-report.json --require-kids-memory-pass --require-kids-preprod-dry-run-pass --require-pass
